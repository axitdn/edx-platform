class @Sequence
  ...
  toggleArrows: =>
    ...
    if @contents.length == 0
      @$('.sequence-nav-button.button-previous').addClass('disabled').attr('aria-hidden', 'true').attr('disabled')
      @$('.sequence-nav-button.button-next').addClass('disabled').attr('aria-hidden', 'true').attr('disabled')
      return

    if @position == 1
      @$('.sequence-nav-button.button-previous').addClass('disabled').attr('aria-hidden', 'true').attr('disabled')
    else
      @$('.sequence-nav-button.button-previous').removeClass('disabled').attr('aria-hidden', 'false').removeAttr('disabled').click(@previous)

    if @position == @contents.length
      @$('.sequence-nav-button.button-next').addClass('disabled').attr('aria-hidden', 'true').attr('disabled')
    else
      @$('.sequence-nav-button.button-next').removeClass('disabled').attr('aria-hidden', 'false').removeAttr('disabled').click(@next)
